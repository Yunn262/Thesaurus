...
def analyze_market(df: pd.DataFrame):
    ...
    projected_move = np.clip((trend_strength * 2) - (volatility if pd.notna(volatility) else 0), -0.02, 0.02)
    ...
    pred_price = last["close"] * (
        1 + projected_move
        if signal == "SUBIDA 🔼"
        else 1 - projected_move if signal == "DESCIDA 🔽" else 1
    )
